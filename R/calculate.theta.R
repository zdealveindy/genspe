...
#' @param beta.div.method,beta.div.sqrt.D Arguments for the function \code{beta.div}, if the \code{method = 'beta.div'}. See Details.
...
#' @param sp Internal argument; name of the species for which the current calculation is done.
#' @param select.spp Internal argument.
...
#' @export
calculate.theta <- function (input.matrix, species.data = NULL, psample = 5, reps = 10, method = "multiplicative", beta.div.method = 'hellinger', beta.div.sqrt.D = FALSE,  beals.file = NULL, pa.transform = FALSE, force.subsample = FALSE, parallel = FALSE, no.cores = 2, remove.out = F, verbal = F, juicer = F, tcltk = F) 
{
  require (tcltk)
  ...
  if (method == 'beta.div' & is.null (beta.div.sqrt.D))  
...
# For beals method, it transforms input.matrix into beals smoothed form:  
  ...
  if (!parallel) 
  {
    ...
    temp.res <- lapply (1:Nspp, FUN = function (sp) calculate.theta.0 (input.matrix = input.matrix, sp = sp, select.spp = select.spp, remove.out = remove.out, psample = psample, reps = reps, method = method, beta.div.method = beta.div.method, beta.div.sqrt.D = beta.div.sqrt.D, beta.div.samp = beta.div.samp, force.subsample = force.subsample, parallel = parallel, win.pb = win.pb, verbal = verbal, juicer = juicer))
    if (verbal) close (win.pb)
  }
  ...
  if (parallel)
  {
    ...
    temp.res <- parLapply (workers, 1:Nspp, fun = function (sp) calculate.theta.0 (input.matrix = input.matrix, sp = sp, select.spp = select.spp , remove.out = remove.out, psample = psample, reps = reps, method = method, beta.div.method = beta.div.method, beta.div.sqrt.D = beta.div.sqrt.D, beta.div.samp = beta.div.samp, force.subsample = force.subsample, parallel = parallel, win.pb = NULL, verbal = verbal, juicer = juicer))
    stopCluster (workers)
  }

  theta.out <- as.data.frame (lapply (1:length (temp.res[[1]]), FUN = function (i) unlist (lapply (temp.res, FUN = function (j) j[[i]]))))
  names (theta.out) <- c ('sci.name', 'local.avgS', 'occur.freq', 'meanco', 'meanco.sd', 'meanco.u', 'meanco.l', 'theta', 'theta.sd')
...
#' @export
#' 
calculate.theta.0 <- function (input.matrix, sp, select.spp, remove.out, psample, reps, method, beta.div.method, beta.div.sqrt.D, beta.div.samp, force.subsample, parallel, win.pb, verbal, juicer)
{
