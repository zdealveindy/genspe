...
calculate.theta <- function (input.matrix, species.data = NULL, reps = 10, psample = 5, method = "multiplicative", beals.file = NULL, parallel = F, no.cores = 2, remove.out = F, verbal = F, juicer = F, tcltk = F) 
{
  ...
  if (is.null (row.names (input.matrix))) row.names (input.matrix) <- seq (1, nrow (input.matrix))
  if (juicer) input.matrix <- ifelse (input.matrix > 0, 1, 0) else
  {
    uni <- unique (as.vector (input.matrix))
    if (!(length (uni) == 2 & all (uni[1:2] == c(0,1))))  
    {
    input.matrix <- ifelse (input.matrix > 0, 1, 0) # checks whether input matrix is presence/absence and if not transforms it and returns warnings - in case of juicer = TRUE it transforms data automatically
    warning ('input.matrix have been transformed into presence/absence form')
    }
  }
...
# For beals method, it transforms input.matrix into beals smoothed form:  
  ...
  names (theta.out) <- c ('sci.name', 'local.avgS', 'occur.freq', 'meanco', 'meanco.sd', 'meanco.u', 'meanco.l', 'GS', 'GS.sd')
...
calculate.theta.0 <- function (input.matrix, species.data, sp, select.spp, remove.out, psample, reps, method, parallel, win.pb, verbal, juicer)
{
  ...
  if (!nrow (temp.matrix) < psample)  # not sure why there is this condition
  {
    ...
    result <- list(sci.name,local.avgS,occur.freq,meanco,meanco.sd,meanco.u,meanco.l,GS,GS.sd)
    return (result)
  }
}
